 ...
 *     stream: the frame rate (in Hz), peak possible sample magnitude (from zero
...
public:
	void Update(uint32_t frame_rate,
	            ...
	            uint8_t expansion_phase_ms = 15u,
	            uint8_t expire_after_seconds = 10u);
	void Process(bool is_stereo, bool is_interpolated, intptr_t prev[], intptr_t next[])
	{
